    ...
    testList
        ...
        [ testCase "Full match simulation produces events"
          ...
          <| fun () ->
              ...
              let output = PlayerAgent.agent ctx s clock
          ...
          <| fun () ->
              ...
              let output = BallAgent.agent ctx s clock
          ...
          <| fun () ->
              ...
              for _ in 1..6 do
                  BallAgent.agent ctx s clock |> ignore

              match s.Ball.Possession with
              | Possession.SetPiece _ -> failtest "Engine is stuck: SetPiece persists after 6 physics ticks."
              | _ -> ()
          ...
          <| fun () ->
              ...
              match s.Ball.Possession with
              | Possession.Owned(HomeClub, 1) -> ()
              | _ -> failtestf $"Sticky Ball failed: Player at 0.5m did not pick up ball. State: %A{s.Ball.Possession}"
