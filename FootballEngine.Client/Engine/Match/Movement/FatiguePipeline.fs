...
module FatiguePipeline =
    ...
    let decisionQuality (player: Player) (profile: BehavioralProfile) condition =
        ...
        let positioningWeight = profile.DefensiveHeight * 0.35 + profile.PressingIntensity * 0.20 + 0.10
        let composureWeight = profile.RiskAppetite * 0.30 + (1.0 - profile.Directness) * 0.20 + 0.15
