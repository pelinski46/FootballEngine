...
module MatchReferee =
    ...
    let cardProbability  (p: Player) = 0.010 + float p.Mental.Aggression * 0.0001
    let injuryProbability(p: Player) = 0.0008 + float (100 - p.Physical.Strength) * 0.00002
    ...
    let processCard (player: Player) (clubId: ClubId) (second: int) (s: MatchState) : MatchState =
        ...
        let ev t = addEvent { Second = second; PlayerId = player.Id; ClubId = clubId; Type = t }

        if count >= 1 then s' |> ev YellowCard |> ev RedCard
        else               s' |> ev YellowCard

    let processInjury (player: Player) (clubId: ClubId) (second: int) (s: MatchState) : MatchState =
        let isHome = clubId = s.Home.Id
        let ts     = side isHome s
        withSide isHome { ts with Sidelined = Map.add player.Id SidelinedByInjury ts.Sidelined } s
        |> addEvent { Second = second; PlayerId = player.Id; ClubId = clubId; Type = Injury "match" }
