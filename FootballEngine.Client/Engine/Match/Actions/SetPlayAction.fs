...
module SetPlayAction =
    ...
    // Phase 5: JIT — kicker resolved from current positions
    let resolveFreeKick (subTick: int) (s: MatchState) : MatchState * MatchEvent list =
        ...
        let gk =
            ...
            |> fun ts -> ts.Players |> Array.tryFind (fun p -> p.Position = GK)
        ...
        if scored then
            let s1, goalEvents = awardGoal ctx.AttSide (Some kicker.Id) subTick s
            let s2 = { s1 with Ball = { s1.Ball with Spin = spin } }
            s2, (event subTick kicker.Id clubId (FreeKick true)) :: goalEvents
        else
            // Phase 2: target position in metres via PhysicsContract
            let targetX =
                if ctx.Dir = LeftToRight then BalanceConfig.FreeKickTargetX
                else PhysicsContract.PitchLength - BalanceConfig.FreeKickTargetX

            let s' =
                s
                |> ballTowards targetX bY BalanceConfig.FreeKickSpeed BalanceConfig.FreeKickVz
                |> flipPossessionAndClearOffside ctx.AttSide
                |> fun s'' -> { s'' with Ball = { s''.Ball with Spin = spin } }
            ...
            let s'' =
                if bernoulli BalanceConfig.PostShotClearProbability then
                    ...
                    s' |> ballTowards PhysicsContract.HalfwayLineX clearY 16.0 1.5
                else
                    s'
    ...
    // Phase 2: all coordinates in metres
    let resolveCorner (subTick: int) (s: MatchState) : MatchState * MatchEvent list =
        let ctx = ActionContext.build s
        ...
        if attSide.Players.Length = 0 then
            ...
            let attackersInBox =
                ...
                |> Array.filter (fun (p, sp, _) ->
                    (p.Position = ST || p.Position = AML || p.Position = AMR
                     || p.Position = AMC || p.Position = MC || p.Position = DC)
                    && (if ctx.Dir = LeftToRight then sp.X > boxThreshold
                        else sp.X < (PhysicsContract.PitchLength - boxThreshold)))

            let defendersInBox =
                outfieldRoster defSide
                |> Array.filter (fun (_, sp, _) ->
                    if ctx.Dir = LeftToRight then sp.X > defBoxThreshold
                    else sp.X < (PhysicsContract.PitchLength - defBoxThreshold))

            let gk = defSide.Players |> Array.tryFind (fun p -> p.Position = GK)

            if attackersInBox.Length = 0 then
                let targetX =
                    if ctx.Dir = LeftToRight then PhysicsContract.PenaltyAreaDepth
                    else PhysicsContract.PitchLength - PhysicsContract.PenaltyAreaDepth

                let s' =
                    s
                    |> ballTowards targetX (PhysicsContract.PitchWidth / 2.0) BalanceConfig.CornerSpeed BalanceConfig.CornerVz
                    |> flipPossessionAndClearOffside ctx.AttSide

                s', [ event subTick attSide.Players[0].Id clubId Corner ]
            else
                ...
                let attackScore =
                    PhysicsContract.normaliseAttr (min 20 (bestAttacker.Physical.Strength + bestAttacker.Technical.Heading))
                    * physicalVariation bestAttackerCond

                let defScore =
                    bestDefender
                    |> Option.map (fun (d, _, _) ->
                ...
                let gkBonus =
                    gk
                    |> Option.map (fun g ->
                        effectiveStat g.Goalkeeping.Reflexes g.Condition g.Morale 1.0 / 100.0)
                    |> Option.defaultValue 0.0

                let numDefenders = float (max 1 defendersInBox.Length)
                let densityPenalty = (numDefenders - 3.0) * 0.05

                // Phase 3: normaliseAttr for crosser
                let crossQuality =
                    attSide.Players
                    |> Array.tryFind (fun p ->
                        p.Position = ML || p.Position = MR || p.Position = AML
                        || p.Position = AMR || p.Position = MC)
                    |> Option.map (fun p ->
                        PhysicsContract.normaliseAttr p.Technical.Crossing * BalanceConfig.CrossCrossingWeight
                        + PhysicsContract.normaliseAttr p.Technical.Passing * BalanceConfig.CrossPassingWeight)
                ...
                if scored then
                    let s1, goalEvents = awardGoal ctx.AttSide (Some bestAttacker.Id) subTick s
                    s1, (event subTick bestAttacker.Id clubId Corner) :: goalEvents
                else
                    let targetX =
                    ...
                    s', [ event subTick bestAttacker.Id clubId Corner ]

    // Phase 5: JIT — nearest teammate resolved at execution time
    let resolveThrowIn (subTick: int) (s: MatchState) (throwClub: ClubSide) : MatchState * MatchEvent list =
        let ctx = ActionContext.build s
        ...
        if attSide.Players.Length = 0 then
            ...
            | Some(teammate, teammateId, _) ->
                // Phase 2: target in metres
                let targetX =
                    if ctx.Dir = LeftToRight then PhysicsContract.PenaltyAreaDepth / 2.0
                    else PhysicsContract.PitchLength - PhysicsContract.PenaltyAreaDepth / 2.0
                ...
                s', [ event subTick thrower.Id clubId (PassCompleted(thrower.Id, teammate.Id)) ]
            | None -> s, []
    ...
    let resolvePenalty
        ...
        let pressNoise = pressureNoise kicker.Mental.Composure BalanceConfig.PenaltyComposureNoise
        ...
        let score =
            ...
            + (if kickerClub = HomeClub then BalanceConfig.HomePenaltyBonus else 0.0)
        ...
        if scored then
            let s1, goalEvents = awardGoal kickerClub (Some kicker.Id) penaltySubTick s
            let penaltyEvent = event penaltySubTick kicker.Id clubId (PenaltyAwarded scored)
            s1, goalEvents @ [ penaltyEvent ]
        else
            // Phase 2: kick-off position in metres
            let s' =
                { s with
                    Ball =
                        { Position = defaultSpatial PhysicsContract.HalfwayLineX (PhysicsContract.PitchWidth / 2.0)
                          Spin = Spin.zero
                          ControlledBy = None
                          LastTouchBy = None
                          IsInPlay = true } }
