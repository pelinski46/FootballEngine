...
module SetPlayAction =
    ...
    let resolveFreeKick (subTick: int) (ctx: MatchContext) (state: SimState) (clock: SimulationClock) : ActionResult =
        ...
        | ValueSome kickerIdx ->
            let kicker =
                let takerResult = SetPieceTakerSelection.selectTaker attRoster.Players SetPieceTakerSelection.scoreFreeKickTaker
                attRoster.Players |> Array.tryFind (fun p -> p.Id = takerResult.PlayerId) |> Option.defaultValue attRoster.Players[kickerIdx]
            ...
            ActionResult.ofEvents [ createEvent subTick kicker.Id clubId (FreeKick false) ]
    ...
    let resolveCorner (subTick: int) (ctx: MatchContext) (state: SimState) (clock: SimulationClock) : ActionResult =
        ...
        if activeAtts.Length = 0 then ActionResult.empty
            ...
            let taker =
                let takerResult = SetPieceTakerSelection.selectTaker activeAtts SetPieceTakerSelection.scoreCrossingTaker
                activeAtts |> Array.tryFind (fun p -> p.Id = takerResult.PlayerId) |> Option.defaultValue activeAtts[0]
    ...
    let resolveThrowIn (subTick: int) (ctx: MatchContext) (state: SimState) (throwClub: ClubSide) (clock: SimulationClock) : ActionResult =
        let actx = ActionContext.build ctx state
        let spc = ctx.Config.SetPiece
    ...
    let resolvePenalty
        ...
        let bX = state.Ball.Position.X
        let distToGoal = abs (goalX - bX)
        let flightTime = if float speed > 0.0 then float distToGoal / float speed else 0.5
