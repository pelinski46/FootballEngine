...
module CornerRoutineSelection =
    ...
    let selectDefense (defenders: Player[]) : CornerDefense =
        let avgMarking = defenders |> Array.averageBy (fun d -> float d.Technical.Marking / 20.0)
        let avgPositioning = defenders |> Array.averageBy (fun d -> float d.Mental.Positioning / 20.0)
        if avgPositioning > 0.6 then
            ZonalMarking 3
        elif avgMarking > 0.6 then
            ManMarking
        else
            Mixed 2
