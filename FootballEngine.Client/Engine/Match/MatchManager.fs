...
module MatchManager =
    ...
    let private conditionThreshold = function
        | Losing -> 75 | Drawing -> 65 | Winning -> 55

    let private preferredPositions = function
    ...
    let private pickIncoming (squadPlayers: Player list) (manager: Staff) (ts: TeamSide) (preferred: Position list) : Player option =
        let startingIds =
    ...
    let decide (second: int) (homeSquad: Player list) (awaySquad: Player list) (s: MatchState) : ManagerIntent list =
        ...
        if not isSubMinute then [ ManagerIdle ]
        else
            ...
            if ts.SubsUsed >= maxSubs then s, []
                ...
                | Some outIdx ->
                    ...
                    | Some incoming ->
                        ...
                        let ts' =
                            { ts with Players = Array.append ts.Players [| incoming |]
                        ...
                        [ { Second = second; PlayerId = playerOut.Id; ClubId = clubId; Type = SubstitutionOut }
                          { Second = second; PlayerId = incoming.Id; ClubId = clubId; Type = SubstitutionIn } ]
