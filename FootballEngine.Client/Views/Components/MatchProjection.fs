...
module MatchProjection =
    ...
    let private shortName (name: string) =
        ...
        if parts.Length >= 2 then parts[parts.Length - 1]
        else name
    ...
    let project (ctx: MatchContext) (snapshot: SimSnapshot) (timeSeconds: float) : RenderFrame =
        ...
        let projectTeam (isHome: bool) (positions: Spatial[]) (players: Player[]) (conditions: int[]) (sidelined: Map<PlayerId, PlayerOut>) =
            positions
            |> Array.mapi (fun i (spatial: Spatial) ->
                let player = if i < players.Length then players[i] else Unchecked.defaultof<Player>
                ...
                {
                    Id = player.Id
                    Position = { X = float spatial.X; Y = float spatial.Y }
                    Velocity = { X = float spatial.Vx; Y = float spatial.Vy }
                    Acceleration = Vector2.zero
                    State = deriveVisualState { X = float spatial.Vx; Y = float spatial.Vy }
                    JerseyNumber = i + 1
        ...
        let homePlayers = projectTeam true snapshot.HomePositions ctx.HomePlayers snapshot.HomeConditions snapshot.HomeSidelined
        let awayPlayers = projectTeam false snapshot.AwayPositions ctx.AwayPlayers snapshot.AwayConditions snapshot.AwaySidelined

        let renderBall =
            {
                Position = { X = float snapshot.BallX; Y = float snapshot.BallY }
                Velocity = { X = float snapshot.BallVx; Y = float snapshot.BallVy }
                Spin = { X = float snapshot.BallSpinSide; Y = float snapshot.BallSpinTop }
                Height = float snapshot.BallZ
            }

        let attackingClubId =
            match snapshot.Possession with
            | Owned(club, _) | InFlight(club, _) | SetPiece(club, _) ->
                if club = HomeClub then Some ctx.Home.Id else Some ctx.Away.Id
            | Loose -> None
...
module MatchInterp =
    ...
    let hermite (frame0: RenderFrame) (frame1: RenderFrame) (t: float) (dt: float) : RenderFrame =
        let players =
            if frame0.Players.Length <> frame1.Players.Length then frame0.Players
            else
                Array.map2 (fun (p0: RenderPlayer) (p1: RenderPlayer) ->
                    { p0 with
                        ...
                        Velocity =
                            ...
                            {
                                X = dh00 * p0.Position.X / dt + dh10 * p0.Velocity.X + dh01 * p1.Position.X / dt + dh11 * p1.Velocity.X
                                Y = dh00 * p0.Position.Y / dt + dh10 * p0.Velocity.Y + dh01 * p1.Position.Y / dt + dh11 * p1.Velocity.Y
                            }
                        Acceleration =
                            {
        ...
        let ball =
            ...
            let b1 = frame1.Ball
            {
                ...
                Velocity =
                    ...
                    let dh11 = 3.0 * t2 - 2.0 * t
                    {
                        X = dh00 * b0.Position.X / dt + dh10 * b0.Velocity.X + dh01 * b1.Position.X / dt + dh11 * b1.Velocity.X
                        Y = dh00 * b0.Position.Y / dt + dh10 * b0.Velocity.Y + dh01 * b1.Position.Y / dt + dh11 * b1.Velocity.Y
                    }
